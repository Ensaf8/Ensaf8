...
public class MultiSelectionSpinner extends androidx.appcompat.widget.AppCompatSpinner implements
    ...
    public void setItems(List<BookMarkType> bookMarkTypeList){
        List<String> items = new ArrayList<>();
        this.bookMarkTypeList = bookMarkTypeList;
        for(BookMarkType bookMarkType:bookMarkTypeList){
            items.add(bookMarkType.getTitle());
        }
    ...
    public String getSelectedItemsAsStringID() {
        ...
        for (int i = 0; i < bookMarkTypeList.size(); ++i) {
            if (mSelection[i]) {
                ...
                foundOne = true;
                sb.append(bookMarkTypeList.get(i).getId());
            }
        }
        sb.append(")");
        return sb.toString();
